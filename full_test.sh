#!/bin/bash
# Comprehensive API test after all 3 fixes
set -e
...
curl -s --max-time 3 http://localhost:8080/.well-known/jwks.json
echo ""

echo ""
echo "=== Server logs (non-PG) ==="
cat /tmp/ueadmin_logs/drogon*.log 2>/dev/null | grep -v 'Pg connection' | tail -20

echo ""
echo "=== Server stdout ==="
cat /tmp/ueadmin_server.log 2>/dev/null | tail -20
